...
const EditUser = () => {
    ...
    const handleClick = async (e) => {
        ...
        let response = 200;
        try {
            response = await axios.put(window.entryPoint + '/user/modUser', user, {headers : headers});
    ...
    return (
        <div className="edit-user-container" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
            <Card title={`Edit user ${unameVal}`} style={{ width: '400px' }}>
                <Toast ref={toast} />
